...
const createRange = (fd) => {
  const outerWrapper = createFieldWrapper(fd, "outerdiv");
  const label = createLabel(fd);
  const dataDiv = createFieldWrapper(fd, "data");
  dataDiv.appendChild(label);
  const inputDiv = createFieldWrapper(fd, "inputDiv");
  ...
  const fieldWrapperParent = createFieldWrapper(fd, "rangediv");
  // fieldWrapper.append(field);
  ...
  // fieldWrapperParent.classList.add('selection-wrapper');
  return { outerWrapper };
};
